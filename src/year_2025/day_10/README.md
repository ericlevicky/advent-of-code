...
- Indicator lights (part 1) form a system over $\mathrm{GF}(2)$; modeling buttons as matrix columns and running Gaussian elimination keeps the search space tiny.
- Joltage (part 2) is a completely different problem: buttons *add* counters, so you must solve $Ax = b$ over the non-negative integers, not modulo 2.
- Naive floating point Gaussian elimination or linear solvers drift off-integer on the large real input. The fix was to work with `fractions.Fraction`, build the reduced row echelon form exactly, and enumerate only the free variables with a bounded DFS.
- Bounds derived from the physical counters (min target value touched by a button) are safe; tighter bounds taken from pivot rows can be overly aggressive and prune real solutions—this was the culprit behind our earlier wrong answer.
- For very small joltage sums (<= 50), a Dijkstra search on counter states is faster than building the full matrix, so the solver uses that shortcut automatically.

### Technique cheat sheet

- **Gaussian elimination (part 1):** represent the button/light relationships as a matrix $A$ with entries in $\{0,1\}$. Appending the target state $b$ gives the augmented matrix $[A|b]$. Repeatedly swap rows so each column has a "pivot" (a 1), then zero out the rest of that column using XOR (because we're in $\mathrm{GF}(2)$). Once every pivot column has only a single 1, you can read off which buttons must be pressed; any column without a pivot is "free" and can be toggled to minimize total presses.
- **Reduced row echelon form (part 2):** with joltage counters we're solving over the integers, so we do the same elimination but using exact fractions. Driving the matrix all the way to RREF (each pivot column has a single 1 and zeros elsewhere) isolates the forced button counts. The remaining free variables are assigned via a bounded depth-first search, and every candidate assignment is plugged back into the pivot equations to keep the solution exact.
- **Dijkstra search shortcut:** when the total required joltage increments is tiny, it's cheaper to brute-force counter states. Each node in the search represents the current counter tuple, edges correspond to pressing a button (cost 1), and Dijkstra guarantees the first time we pop the goal state we've used the fewest presses because all edge weights are non-negative and equal.

### Kid-friendly story

- The lights puzzle is like a bunch of toy switches that flip groups of lights. Since flipping a light twice turns it back off, we only care whether each button is pressed an even or odd number of times. We write all the switches in a neat table and use a tidy procedure (Gaussian elimination) to figure out the shortest list of button presses that makes every light match the picture.
- The joltage puzzle uses the very same switches, but now every press makes some counters go up by one. We want the counters to hit exact numbers without ever going down. Instead of guessing, we carefully write another table, do perfect fraction math so nothing gets rounded, and let the table tell us how many times each button must be pressed. If some buttons can take different values, we try the smallest options first until the counters line up exactly.
- When the goal numbers are super tiny, we don’t bother with tables. We simply try every reachable counter combination using Dijkstra’s algorithm, which is like always taking the shortest path through a maze of button presses.
